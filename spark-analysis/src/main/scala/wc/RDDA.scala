...
object RDDA {
  ...
  def main(args: Array[String]) {
    ...
    // RDD-A: AggregateByKey to count followers
    val RDDAfollowerCount = twitterDataRDD
      .map(x => (x(1), 1))
      .aggregateByKey(0)(_ + _, _ + _)

    RDDAfollowerCount.saveAsTextFile(args(1))

    println(RDDAfollowerCount.toDebugString)
