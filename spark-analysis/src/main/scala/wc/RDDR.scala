...
object RDDR {
  ...
  def main(args: Array[String]) {
    ...
    // RDD-R: ReduceByKey to count followers
    val RDDRfollowerCount = twitterDataRDD
      .map(x => (x(1), 1))
      .reduceByKey(_ + _)

    RDDRfollowerCount.saveAsTextFile(args(1))

    println(RDDRfollowerCount.toDebugString)
